...
Diagram: `setup()` → pierwsze cykle `loop()` → koniec kalibracji → gotowość do pracy. **Bez WebDebug** — panel serwisowy opisano osobno w § 8.4.
...
**Poza diagramem:** kabel tylko do ładowania → gałąź „Nie” przy USB (BT włączony); BT może uśnąć po 5 min bez ruchu; mrugnięcia i HID dopiero po `systemGotowy`.
